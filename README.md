...
`gpuRIR` is used in the notebook to simulate the acoustic propagation from the noise source to the reference microphones and error microphone. This is the primary/noise path and changes with source location, room geometry, and reverberation time.

`SecondaryPath_final.mat` serves a different role. It stores `Sec_path_cal`, a 256-tap secondary-path impulse response from the control loudspeaker to the error microphone. The FxLMS filtered-x signal and the fixed-filter ANC controller both need this same secondary path, and the pre-trained control filters were generated for this path.

For that reason, the secondary path is kept as a separate asset instead of being regenerated by `gpuRIR` during every run. If the secondary path is changed, the pre-trained control filters should be regenerated as well.
